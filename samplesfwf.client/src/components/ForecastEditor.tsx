...
type FieldErrors = Partial<Record<'date' | 'temperatureC' | 'summary' | 'form', string[]>>;

/**
 * Helper: normalize and extract ValidationProblemDetails-like payloads.
 * Accepts JSON object or string; returns { errors?, title?, detail? }.
 */
function normalizeServerBody(raw: any) {
  // Try to convert JSON string into an object
  let body = raw;
...
export default function ForecastEditor({ open, initial, onClose, onSaved }: ForecastEditorProps) {
  ...
  const mapServerErrors = (rawBody: any): FieldErrors => {
    const body = normalizeServerBody(rawBody);
    const result: FieldErrors = {};

    if (!body) return result;

    // If it's a ValidationProblemDetails-style object
    // shape: { errors: { "Date": ["..."], "TemperatureC": ["..."] }, title, detail }
    const errorsObj = body.errors ?? (typeof body === 'object' ? body : undefined);
    if (errorsObj && typeof errorsObj === 'object') {
      for (const [key, messages] of Object.entries(errorsObj)) {
        ...
        else if (normalizedKey === 'temperaturec' || normalizedKey === 'temperaturec') result.temperatureC = msgs;
        else if (normalizedKey === 'summary') result.summary = msgs;
        else {
          // form-level or unknown key => attach to 'form'
          result.form = (result.form ?? []).concat(msgs);
        }
      }
    }

    // If no field-specific errors found, try title/detail/message fields
    if (!result.date && !result.temperatureC && !result.summary) {
      const detail = (body && (body.detail || body.error || body.message)) ?? undefined;
